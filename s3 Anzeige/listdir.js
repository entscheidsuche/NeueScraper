...
function renderRowBuf(item) {
	var name=item.keyText.split('.')[0];
	var row='';
	if(in_store.length>0 && in_store_name==name){
		in_store.push(item);
		return(row);
	}
	if(in_store.length>0 && in_store_name!=name){
...
function renderRow() {
  if(in_store.length>0){
	  ...
	  if(in_store.length==1){
		...
		var parts=in_store[0].keyText.split('_');
		var pre='<td rowspan="'+in_store.length.toString()+'">';
		row += pre+"<img src='https://entscheidsuche.ch/img/"+parts[0].toLowerCase()+".svg' height='25px'></td>";  	
		row += pre+parts[0]+"_"+parts[1]+"_"+parts[2]+"</td>";
		row += pre+parts[3]+"</td>";
		row += pre+parts[4].split(".")[0]+"</td>";
		while (in_store.length>0){
			var item=in_store.shift();
			typ = item.keyText.split(".")[1];
